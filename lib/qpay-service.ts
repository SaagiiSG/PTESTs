...
const QPAY_COURSE_CONFIG = {
  client_id: process.env.QPAY_COURSE_CLIENT_ID || 'JAVZAN_B', // Temporarily use test credentials
  client_secret: process.env.QPAY_COURSE_CLIENT_SECRET || 'fGJp4FEz', // Temporarily use test credentials
  invoice_code: process.env.QPAY_COURSE_INVOICE_CODE || 'JAVZAN_B_INVOICE', // Temporarily use test invoice code
  callback_url: process.env.NODE_ENV === 'development'
    ? 'http://localhost:3000/api/qpay-callback'
    : (process.env.QPAY_CALLBACK_URL || 'https://setgelsudlal-git-main-saagiisgs-projects.vercel.app/api/qpay-callback')
};

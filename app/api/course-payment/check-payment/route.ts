...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('Checking course payment for invoice:', invoiceId);
    ...
    if (storedPayment && storedPayment.payment_status === 'PAID') {
      console.log('Found stored payment data for course payment:', storedPayment);
      return NextResponse.json({
        success: true,
        payment: {
          count: 1,
    ...
    try {
      const qpayService = getCourseQPayService();
      const qpayResult = await qpayService.checkPayment(invoiceId);
      
      console.log('QPay API check result for course payment:', qpayResult);
      
      if (qpayResult && qpayResult.rows && qpayResult.rows.length > 0) {
        const payment = qpayResult.rows[0];
        console.log('Found course payment via QPay API:', payment);
      ...
      console.error('QPay API check failed for course payment:', qpayError);
    ...
    console.error('Course payment check failed:', error);
